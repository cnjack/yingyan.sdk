package yingyan

const (
	apiRootPath = `http://yingyan.baidu.com`

	entityAddPath      = `/api/v3/entity/add`
	entityDeletePath   = `/api/v3/entity/delete`
	entityUpdatePath   = `/api/v3/entity/update`
	entityList         = `/api/v3/entity/list`
	entitySearch       = `/api/v3/entity/search`
	entityAroundSearch = `/api/v3/entity/aroundsearch`
	entityBoundSearch  = `/api/v3/entity/boundsearch`

	trackAddPoint       = `/api/v3/track/addpoint`
	trackAddPoints      = `/api/v3/track/addpoints`
	trackGetLatestPoint = `/api/v3/track/getlatestpoint`
	trackGetDistance    = `/api/v3/track/getdistance`
	trackGetTrack       = `/api/v3/track/gettrack`

	analysisStaypoint = `/api/v3/analysis/staypoint`
)
